  ...
  try {
    const { error } = await resend.emails.send({
      ...
      html: [
        ...
        `<p>Verify the requester controls this address before deleting. Then, in order:</p>`,
        `<p><strong>1.</strong> Delete the Supabase auth user (cascades every owned row + storage).<br/>`,
        `<strong>2.</strong> Delete the matching <code>contacts</code> row on the landing page.<br/>`,
        `<strong>3.</strong> Remove the address from the closed-test tester list in the Play Console.`,
        ` Nothing reaches that list automatically — <code>edits.testers</code> only accepts Google`,
        ` Groups and there is no group — so skipping this step leaves the person with access to`,
        ` the test build after their account is gone.</p>`,
        `<p>The trial_ledger row is retained by design — remove it only on an article 21`,
        ` objection, and do it <em>before</em> deleting the auth user: the provider hash becomes`,
        ` unreachable once <code>auth.identities</code> is gone.</p>`,
      ].join(""),
    });
