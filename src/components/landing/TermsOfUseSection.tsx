...
interface TermsOfUseSectionProps {
  content: Translations["termsOfUse"];
...
export function TermsOfUseSection({ content, t, locale }: TermsOfUseSectionProps) {
      ...
      <div className="mx-auto max-w-[820px] px-8">
        ...
        {locale !== "en" && (
          <div className="mt-8 flex items-start gap-3 rounded-[16px] bg-accent px-5 py-4">
            ...
            <p className="text-[14px] leading-[1.55] text-accent-foreground">{t.termsEnglishNotice}</p>
          </div>
        )}
        ...
        <ol className="mt-4 list-decimal space-y-1.5 pl-5 text-[15px] leading-relaxed text-primary marker:text-muted-foreground">
          {sections.map((s, i) => (
            <li key={i}>
              <a href={`#terms-section-${i + 1}`} className="underline underline-offset-2 hover:text-teal-600">
                {s.title}
              </a>
            </li>
          ))}
        ...
        {/* Numbered sections */}
        {sections.map((s, i) => (
          <section key={i} id={`terms-section-${i + 1}`} className="scroll-mt-24">
            <h2 className="mt-14 font-display text-[24px] font-semibold tracking-[-0.5px] text-foreground">
              {i + 1}. {s.title}
            </h2>
